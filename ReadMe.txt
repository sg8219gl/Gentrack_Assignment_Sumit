...
i)   MeterWebAPI   -  Created a public methods as logic  GetTotalMeterReading(string meterName, DateTime readingDate) and GetAllMeterReadingTotal(DateTime readingDate) which accept metername and redingDate as input parameter return total reading on that day as Output of that meter and all meters on particulare date.
...
iii) MeterDataUnitTest  -  Create a Unit Test MeterController_GetTotalMeterReading() and GetAllMeterReadingTotal() which we mock meter data and return total reading of that Meter on that day and for all meters . It completes in around 120 ms.
